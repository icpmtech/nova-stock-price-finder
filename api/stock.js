...
// Ex.: https://your-project.vercel.app/api/stock?symbol=NOS.LS
// ---------------------------------------------------------------------

// (Em runtimes Node 18+ do Vercel o fetch global já existe; se usar
//  uma versão mais antiga, descomente a linha abaixo.)
// import fetch from 'node-fetch';

export default async function handler(req, res) {
  const { symbol = 'NOS.LS' } = req.query;
  const respondError = (msg, code = 500) =>
    res.status(code).json({ error: msg });

  // Helper: converte unix-epoch (segundos) → AAAA-MM-DD
  const toDate = (unix) =>
    new Date(unix * 1000).toISOString().split('T')[0];

  try {
    // ───── 1. Quote / Summary / Profile / Price Targets ────────────────
    const summaryURL =
    ...
    // ───── 2. Histórico de preços (último ano, diário) ────────────────
    const chartURL =
      `https://query1.finance.yahoo.com/v8/finance/chart/` +
      `${encodeURIComponent(symbol)}?range=1y&interval=1d&includePrePost=false`;
    ...
    // ───── 3. Demonstrativos anuais (receita / lucro líquido) ─────────
    const incomeURL =
      `https://query1.finance.yahoo.com/v10/finance/quoteSummary/` +
      `${encodeURIComponent(symbol)}?modules=incomeStatementHistory`;

    const incomeJson = await fetch(incomeURL).then((r) => r.json());
    const incomeHist =
      incomeJson.quoteSummary?.result?.[0]?.incomeStatementHistory
    ...
    // pega os 4 anos mais recentes (ordenados cronologicamente)
    [...incomeHist]
      .sort((a, b) => a.endDate.raw - b.endDate.raw)
      .slice(-4)
      .forEach((item) => {
        years.push(new Date(item.endDate.raw * 1000).getFullYear());
        revenue.push(Math.round(item.totalRevenue.raw / 1e6)); // € M
        earnings.push(Math.round(item.netIncome.raw / 1e6));   // € M
      });

    // ───── 4. Monta a resposta no formato esperado pelo front-end ─────
    const data = {
      symbol,
      ...
      // Yahoo não fornece pontuação de controvérsia; placeholder (0-5)
      controversy: 1,
    };

    res.setHeader('Access-Control-Allow-Origin', '*'); // CORS livre

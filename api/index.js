...
try { const maxP = toNum(max, 5); const minP = toNum(min, 0); const minVol = toNum(minVolume, 0); const capKey = (cap || '').toLowerCase();

const rawQuotes = await Promise.all(
  DEFAULT_STOCKS.map(t => yahooFinance.quote(t, { modules: ['summaryDetail', 'financialData'] }).catch(() => null))
);

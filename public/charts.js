...
export function renderPortfolioChart(ctx) {
  if (!(walletData.portfolioHistory ?? []).length) return; // nothing to draw
    ...
    data: {
      ...
      datasets: [
        ...
        },
        {
          ...
          borderColor: '#8b5cf6', backgroundColor: 'rgba(139,92,246,0.1)', fill: false, tension: 0.4
        }
      ]
    },
    options: {
...
export function renderIncomeExpensesChart(ctx) {
  if (!(walletData.monthlyData ?? []).length) return;
      ...
      datasets: [
        {
          ...
          backgroundColor: 'rgba(16,185,129,0.8)', borderColor: '#10b981', borderWidth: 1
        },
        {
          label: 'Expenses',
...
export function renderCategoryChart(ctx) {
  if (!walletData.spendingCategories || !Object.keys(walletData.spendingCategories).length) return;
